...
<?php error_reporting(0); ?>
	...
	<div class="box">
		<h1>Tata Tertib</h1>
		<h4>
		<small>Susunan Tata Tertib</small>
		<!-- Membuat Button Tambah Data -->
		<div class="pull-right">
			<a href="" class="btn btn-default btn-xs"><i class="glyphicon glyphicon-refresh"></i></a>
			<a href="add.php" class="btn btn-success btn-xs"><i class="glyphicon glyphicon-plus"></i>Tambah Aturan</a>
		</div>
	</h4> 
		...
		<table class="table table-striped table-bordered table-hover">
			...
			<tbody>
				...
				if ($_SERVER['REQUEST_METHOD'] == "POST" ) {
					...
					if ($pencarian != '') {
						$sql = "SELECT * FROM tb_peraturan WHERE nama LIKE '%$pencarian%'";
						$query = $sql;
						$queryJml = $sql;
					} else {
						$query = "SELECT * FROM tb_peraturan LIMIT $posisi, $batas";
						$queryJml = "SELECT * FROM tb_peraturan";
						$no = $posisi + 1;
					}
				} else {
					$query = "SELECT * FROM tb_peraturan LIMIT $posisi, $batas";
