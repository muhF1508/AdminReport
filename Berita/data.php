...
<?php error_reporting(0); ?>
	...
	<div class="box">
		...
		<div class="pull-right">
			...
			<a href="add.php" class="btn btn-success btn-xs"><i class="glyphicon glyphicon-plus"></i>Tambah Berita</a>
		</div>
	</h4> 
		...
		<table class="table table-striped table-bordered table-hover">
			...
			<tbody>
				...
				if ($_SERVER['REQUEST_METHOD'] == "POST" ) {
					...
					if ($pencarian != '') {
						$sql = "SELECT * FROM tb_berita WHERE judul LIKE '%$pencarian%'";
						$query = $sql;
						$queryJml = $sql;
					} else {
						$query = "SELECT * FROM tb_berita LIMIT $posisi, $batas";
						$queryJml = "SELECT * FROM tb_berita";
						$no = $posisi + 1;
					}
				} else {
					$query = "SELECT * FROM tb_berita LIMIT $posisi, $batas";
